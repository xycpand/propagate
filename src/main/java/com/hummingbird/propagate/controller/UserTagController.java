...
public class UserTagController extends BaseController{
	...
	@AccessRequered(methodName = "查询用户的标签接口", isJson = true, codebase = 245900, className = "com.hummingbird.commonbiz.vo.BaseTransVO", genericClassName = "com.hummingbird.propagate.vo.OpenidBodyVO", appLog = true)
	...
	@AccessRequered(methodName = "查询某文章的标签", isJson = true, codebase = 245901, className = "com.hummingbird.commonbiz.vo.BaseTransVO", genericClassName = "com.hummingbird.propagate.vo.ArticleTagBodyVO", appLog = true)
	...
	@AccessRequered(methodName = "为用户的某篇文章添加标签", isJson = true, codebase = 245903, className = "com.hummingbird.commonbiz.vo.BaseTransVO", genericClassName = "com.hummingbird.propagate.vo.AddArticleTagBodyVO", appLog = true)
	...
	//4. 为用户的某篇文章删除标签
	@RequestMapping(value = "/delArticleTag", method = RequestMethod.POST)
	@AccessRequered(methodName = "为用户的某篇文章删除标签", isJson = true, codebase = 245903, className = "com.hummingbird.commonbiz.vo.BaseTransVO", genericClassName = "com.hummingbird.propagate.vo.AddArticleTagBodyVO", appLog = true)
	public @ResponseBody ResultModel delArticleTag(HttpServletRequest request,
			HttpServletResponse response) {
		ResultModel rm = super.getResultModel();
		BaseTransVO<AddArticleTagBodyVO> transorder = (BaseTransVO<AddArticleTagBodyVO>) super.getParameterObject();
		String messagebase = "为用户的某篇文章删除标签";
		...
		try {
			...
			//添加标签
			userTagService.delArticleTag(transorder.getBody());
			rm.setErrmsg(messagebase+"成功");
		} catch (Exception e1) {
			log.error(String.format(messagebase + "失败"), e1);
			rm.mergeException(e1);
	...
	@AccessRequered(methodName = "查询热门标签", isJson = true, codebase = 245905, className = "com.hummingbird.commonbiz.vo.BaseTransVO", genericClassName = "com.hummingbird.propagate.vo.OpenidBodyVO", appLog = true)
